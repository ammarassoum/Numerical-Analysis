 ...
 */
void rk4(double (*f)(double, double), double t0, double y0, double h, double t_end, 
    ...
    *n_out = n;
    ...
    // Pre-calculate all time steps to avoid cumulative floating point errors
    for (int i = 1; i < n; i++) {
        t[i] = t0 + i * h;
    }
    ...
    // RK4 method loop
    for (int i = 0; i < n - 1; i++) {
        double k1 = h * f(t[i], y[i]);                               // Slope at the beginning
        double k2 = h * f(t[i] + h / 2.0, y[i] + k1 / 2.0);          // Slope at midpoint using k1
        double k3 = h * f(t[i] + h / 2.0, y[i] + k2 / 2.0);          // Slope at midpoint using k2
        double k4 = h * f(t[i] + h, y[i] + k3);                      // Slope at the end
        
        y[i + 1] = y[i] + (k1 + 2.0 * k2 + 2.0 * k3 + k4) / 6.0;     // Weighted average
    }
...
int main() {
    ...
    // Call RK4 method
    rk4(test_func, t0, y0, h, t_end, &t, &y, &num_steps);
    
    // Print the result as a formatted table
    printf("Runge-Kutta 4th Order Results:\n");
    printf("%-10s %-10s\n", "t", "y");
    printf("---------------------\n");
