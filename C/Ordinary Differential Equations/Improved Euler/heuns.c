 ...
 */
void heuns(double (*f)(double, double), double t0, double y0, double h, double t_end, 
           double **t_out, double **y_out, int *n_out) {
    
    // Calculate the total number of points (adding 0.5 prevents floating point truncation issues)
    ...
    *n_out = n;
    ...
    // Heun's method loop
    for (int i = 0; i < n - 1; i++) {
        // Predictor step (Standard Euler's method)
        double y_predict = y[i] + h * f(t[i], y[i]);
        
        // Corrector step (Average the slopes at the beginning and the predicted end)
...
int main() {
    ...
    // Call Heun's method
    heuns(test_func, t0, y0, h, t_end, &t, &y, &num_steps);
    
    // Print the result as a formatted table
    printf("Heun's Method Results:\n");
    printf("%-10s %-10s\n", "t", "y");
    printf("---------------------\n");
