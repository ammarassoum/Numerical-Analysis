 ...
 */
void euler(double (*f)(double, double), double t0, double y0, double h, double t_end, 
           double **t_out, double **y_out, int *n_out) {
    
    // Calculate the total number of points (adding 0.5 to prevent floating point truncation issues)
    ...
    *n_out = n;
    ...
    // Euler's method loop
    for (int i = 0; i < n - 1; i++) {
        t[i + 1] = t0 + (i + 1) * h;               // Generate time step
...
int main() {
    ...
    // Pointers to hold our output arrays and variable for the array size
    double *t = NULL;
    double *y = NULL;
    int num_steps = 0;
    
    // Call the Euler method
    euler(test_func, t0, y0, h, t_end, &t, &y, &num_steps);
    
    // C doesn't have native plotting, so we print the result as a table
    printf("Euler's Method Results:\n");
    printf("%-10s %-10s\n", "t", "y");
    printf("---------------------\n");
